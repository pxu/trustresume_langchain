...
| **HTTP** | `api/server.py` | Routes unchanged in spirit, extended into four resource groups: documents (JSON + multipart upload + delete), jobs (CRUD, job-scoped upload/generate/list), generation and résumés (including PDF/Markdown export), and retrieval/ops (`/api/search`, `/api/ping`, `/api/health`). Every user-scoped route resolves its caller from `X-User-Id` via one dependency (ADR-0014) instead of a hardcoded demo id. Still operates on `WorkflowState`/the facade, not agent internals. |
