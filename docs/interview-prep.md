...
>
> If the score is too low, the system writes specific feedback and rewrites,
> up to three times. It is built on LangChain, LangGraph, and ChromaDB, with a
> FastAPI backend.
...
> increment node runs. So `max_iterations=3` produces **four** drafts, not
> three. That is an off-by-one that is easy to introduce accidentally, so
...
>
> Third, **feedback for rewrites is generated by code, not by an LLM.** When a
> draft fails, the instructions — remove this claim, add that keyword — are
> derived deterministically from the two reports. That is one fewer LLM call,
> and one fewer source of variance inside a loop.
>
...
| Number | What it is |
|---|---|
| 5–9 | LLM calls per generation |
| 6 | agents (4 use an LLM, 2 are pure code) |
| 4 | max drafts (initial + 3 rewrites) |
